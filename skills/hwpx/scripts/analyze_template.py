...
def main():
    ...
    try:
        with zipfile.ZipFile(args.input, 'r') as z:
            # ZipFile.extractall() strips '..' components and absolute paths,
            # so extraction stays inside tmpdir even for crafted archives.
            z.extractall(tmpdir)
        ...
        header_root = etree.parse(header_path, parser=_SAFE_XML_PARSER).getroot()
        section_root = etree.parse(section_path, parser=_SAFE_XML_PARSER).getroot()
